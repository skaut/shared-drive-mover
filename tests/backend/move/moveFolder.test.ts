...
vi.mock("../../../src/backend/move/resolveDestinationFolder");
vi.mock("../../../src/backend/utils/MoveState");

test("moveFolder works correctly with an empty folder", () => {
  const listFilesInFolder = vi
    .mocked(folderManagement)
    .listFilesInFolder_.mockReturnValueOnce([]);
  const listFoldersInFolder = vi
    .mocked(folderManagement)
  ...
  expect(vi.mocked(state).addPath.mock.calls).toHaveLength(0);
  expect(vi.mocked(state).removePath.mock.calls).toHaveLength(1);
  expect(vi.mocked(state).removePath.mock.calls[0][0]).toBe(context);
  expect(vi.mocked(state).saveState.mock.calls).toHaveLength(1);
});
...
test("moveFolder adds subfolders to the state correctly", () => {
  ...
  const resolveDestinationFolderFn = vi
    .mocked(resolveDestinationFolder)
  ...
  expect(vi.mocked(state).addPath.mock.calls).toHaveLength(2);
  expect(vi.mocked(state).addPath.mock.calls[0][0]).toBe("SRC_SUBFOLDER1_ID");
  expect(vi.mocked(state).addPath.mock.calls[0][1]).toBe("DEST_SUBFOLDER1_ID");
  expect(vi.mocked(state).addPath.mock.calls[0][2]).toStrictEqual([
    "PATH",
    "TO",
    "FOLDER",
    "SUBFOLDER1_NAME",
  ]);
  expect(vi.mocked(state).addPath.mock.calls[1][0]).toBe("SRC_SUBFOLDER2_ID");
  expect(vi.mocked(state).addPath.mock.calls[1][1]).toBe("DEST_SUBFOLDER2_ID");
  expect(vi.mocked(state).addPath.mock.calls[1][2]).toStrictEqual([
    "PATH",
    "TO",
    "FOLDER",
    "SUBFOLDER2_NAME",
