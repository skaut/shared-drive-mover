...
test("resolveDestinationFolder corretly creates new folder", () => {
  ...
  driveServiceMock.Files.create.mockReturnValueOnce({
  ...
  ).toStrictEqual({ id: "NEWLY_CREATED_FOLDER_ID", name: "FOLDER_NAME" });
  ...
  expect(driveServiceMock.Files.create.mock.calls[0][3]).toBeDefined();
  expect(
    (driveServiceMock.Files.create.mock.calls[0][3] as InsertFileOptions)
      .supportsAllDrives,
  ).toBe(true);
