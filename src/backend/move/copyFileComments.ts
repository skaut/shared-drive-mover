  ...
  return paginationHelper_<SafeCommentList, SafeComment>(
    (pageToken) =>
      driveService.Comments.list(fileID, {
        fields:
          "nextPageToken, comments(author(isAuthenticatedUser, displayName), content, status, context, anchor, replies(author(isAuthenticatedUser, displayName), content, verb))",
        maxResults: 100,
        pageToken,
      }),
