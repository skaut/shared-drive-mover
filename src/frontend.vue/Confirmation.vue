<template>
  <div>
    ...
    <br />
    <br />
    <md-button class="md-raised md-primary" @click="$emit('next-step')">
      {{ $t("moveButton") }}
    </md-button>
  </div>
</template>
...
export default Vue.extend({
  ...
  computed: {
    folderName(): string {
      ...
      ret += (this.folders as Array<NamedRecord>).find(
        (i) => i.id === this.folder
      )!.name;
      return ret;
    },
    sharedDriveName(): string {
      ...
      ret += (this.sharedDrives as Array<NamedRecord>).find(
        (i) => i.id === this.sharedDrive
      )!.name;
      return ret;
    },
  },
